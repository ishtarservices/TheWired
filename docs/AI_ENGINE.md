...
- **Keys never in Redux** — only `llmManager` memory + keychain (`secretStore`). Slice holds non-secret config only.
- All backends normalize to `ChatChunk`; UI/`streamRunner` never see provider-specific framing.
- AbortControllers + delta buffers live in `streamRunner` (module state), not Redux.
- Streaming deltas are Redux-only; only the FINAL assistant message is persisted (`putMessage`).
...
- WRITE (gated): `publish_note`/`reply_to`/`send_dm`/`post_to_space`/`publish_article`. `run()` builds NOTHING signed — it registers a `PendingWrite` (id = toolCallId, binding approval to the exact call) and returns an "awaiting approval" tool result. `gate/PendingWriteCard.tsx` shows the exact draft + target (Intent Preview) with **Approve / Edit / Cancel**; `gate/approveWrite.ts` resolves relays/recipients app-side and signs via `signAndPublish`/`sendDM` ONLY on Approve (no optimistic publish; "Waiting for signer…" pending state for NIP-46; reports relay count). Max 3 open pending per conversation.
...
- **Deferred from the audit:** message-list **virtualization** (large convos — DOM size, not re-renders); model-picker **arrow-key** nav (Tab+Escape done); **avatar grouping** of consecutive turns (timestamps done); `pendingWrites` persist/prune (resolved cards kept as session receipts, cleaned on conversation delete); artifact **versioning** (single-version per `${messageId}#${i}`).
