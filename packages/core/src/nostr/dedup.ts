 ...
 *
 * At 100k entries of 64-char hex IDs, memory usage is ~15 MB — fine for
 * desktop; mobile callers pass a smaller capacity.
 */
const DEFAULT_LRU_CAPACITY = 100_000;

export class EventDeduplicator {
  private seen: LRUCache<string, true>;

  constructor(capacity: number = DEFAULT_LRU_CAPACITY) {
    this.seen = new LRUCache<string, true>({ max: capacity });
  }
