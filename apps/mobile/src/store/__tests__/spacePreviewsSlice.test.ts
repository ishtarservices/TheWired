...
const msg = (over: Partial<Parameters<typeof channelPreviewUpserted>[0]> = {}) =>
  channelPreviewUpserted({
    spaceId: "s1",
    channelId: "general",
    createdAt: 100,
...
describe("spacePreviewsSlice", () => {
  it("upserts a preview and keeps the newest message", () => {
    ...
    expect(preview.eventTimestamps).toEqual(expect.arrayContaining([90, 100]));
  });
  ...
  it("bounds the per-channel timestamp list", () => {
    let state: SliceState | undefined;
    for (let i = 0; i < 150; i++) {
      state = reduce(state, msg({ createdAt: 1000 + i, content: `m${i}` }));
    }
    const preview = selectChannelPreview(wrap(state!), "s1", "general")!;
    expect(preview.eventTimestamps.length).toBeLessThanOrEqual(100);
