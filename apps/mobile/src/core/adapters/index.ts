...
// (mobile-guide/01-architecture.md §3). TYPES ONLY — the mobile
// implementations live in src/platform/, and these definitions move to
// packages/core/src/adapters at Phase 0; this file then becomes a re-export
// of `@thewired/core/adapters`. Keep it dependency-free apart from
...
/** Local persistence. Desktop: IndexedDB (idb). RN: SQLite (op-sqlite). */
...
/**
 * Signs events + NIP-44. Same contract as the desktop NostrSigner
 * (client/src/lib/nostr/signer.ts). Desktop: Tauri keystore / NIP-07 /
 * NIP-46. Mobile v1: NIP-46 first, native keystore later (guide 00).
 * All implementations must be driven through the signing queue.
 */
