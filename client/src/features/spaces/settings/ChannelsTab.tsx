    ...
    <div className="space-y-4">
      ...
      <div className="space-y-1">
        {sorted.map((ch) => (
          <div
            key={ch.id}
            className="flex items-center gap-2 rounded-xl px-2 py-2 hover:bg-surface-hover transition-colors group"
          >
            ...
            {editingId === ch.id ? (
              <input
                ...
                autoFocus
                className="flex-1 rounded-xl bg-field border border-edge px-2 py-0.5 text-sm text-heading focus:border-neon focus:outline-none"
              />
            ) : (
              <button
