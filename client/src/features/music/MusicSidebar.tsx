    ...
    <div className="flex flex-1 flex-col">
      <div className="space-y-0.5 p-2">
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              key={item.view}
              onClick={() => dispatch(setMusicView(item.view))}
              className={clsx(
      ...
      <div className="mt-auto p-2">
        <button
          ...
          className="flex w-full items-center gap-2 rounded-md border border-dashed border-edge px-2 py-1.5 text-sm text-soft transition-colors hover:border-heading hover:text-heading"
        >
          <Upload size={16} />
          <span>Upload</span>
        </button>
